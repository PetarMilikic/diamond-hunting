...
const float XCOORD = -3.3f;
const float YCOORD = -0.3f;
const float ZCOORD =  3.3f;
...
public:
    ...
    Model(string const &path, bool gamma = false, float xcoord = XCOORD, float ycoord = YCOORD,float zcoord = ZCOORD)
    : gammaCorrection(gamma), XCoord(xcoord), YCoord(ycoord), ZCoord(zcoord)
    {
        loadModel(path);
    }
